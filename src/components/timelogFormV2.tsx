...
const TimeLogFormV2 = ({ formData, handleFormData }: Props) => {
  ...
  const [myProject, setMyProject] = useState<any>([]); // Consolidate with allProjects?
  const [orgProject, setOrgProject] = useState<any>([]); // Consolidate with allProjects?
  const [allProjects, setAllProjects] = useState<any>([]);
  const [projectArr, setProjectArr] = useState<any>([]); //Not sure
  const [projectList, setProjectList] = useState<any>([]);
  const [milestoneList, setMilestoneList] = useState<any>([]);
  const [taskList, setTaskList] = useState<any>([]);
  const [selected, setSelected] = useState<any>([]); // Store selected Project, Milestones & Tasks
  const [selectedProject, setSelectedProject] = useState<string | undefined>();
  const [selectedMilestone, setSelectedMilestone] = useState<string | undefined>();
  const [selectedTask, setSelectedTask] = useState<string | undefined>();
  const [isAllDropDownSelect, setAllDropDownSelect] = useState(false); // This can be removed - Since only setting when project, milestone & task is set Then show comment field
    ...
    >
      <form onSubmit={(e) => handleSubmit(e)} onKeyDown={(e) => e.keyCode === 13 && handleSubmit(e)}>
        <Command label="Command Menu" className="relative text-content-light">
          ...
          >
            {/* Check if Project / Milestone / Task is seledted then show Comment field else select dropdown for Project / Milstone / Task */}
            {selected?.projectName && selected?.milestoneName && selected?.taskName && selected ? (
