...
const LeavesEmail = ({ subject, data }: LeavesEmailProps) => {
  return (
    <>
      <Preview>Please Find {subject.toLowerCase()}</Preview>
      <Tailwind>
        <div key="email-body" className="mx-auto my-auto bg-white px-2 font-sans">
          <div className="mx-auto my-[40px] max-w-[465px] rounded border border-solid border-[#eaeaea] p-[20px]">
            <Heading className="mx-0 my-[30px] p-0 text-center text-[24px] font-normal text-black">{subject}</Heading>
            <Text className="text-[14px] leading-[24px] text-black">Hi {data.name},</Text>
            <Text className="text-[14px] leading-[24px] text-black">{JSON.stringify(data, null, 2)}</Text>
