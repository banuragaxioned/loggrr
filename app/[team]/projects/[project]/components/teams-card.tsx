    ...
    >
      {hoveredIndex === item.id && (
        <motion.div
          ...
          className="absolute -left-[85%] -top-10 z-50 flex translate-x-1/2 flex-col items-center justify-center rounded-md bg-primary px-2 py-1.5 text-xs shadow-xl"
        >
          <div className="absolute inset-x-10 -bottom-px z-30 h-px w-[20%] bg-gradient-to-r from-transparent via-emerald-500 to-transparent " />
          <div className="absolute -bottom-px left-10 z-30 h-px w-[40%] bg-gradient-to-r from-transparent via-sky-500 to-transparent " />
          <div className="relative z-30 min-w-[100px] text-center text-xs text-secondary">{item.name}</div>
        </motion.div>
      )}
