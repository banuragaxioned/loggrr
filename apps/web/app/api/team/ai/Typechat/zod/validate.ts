 ...
 */
export function createZodJsonValidator<T extends Record<string, z.ZodType>, K extends keyof T & string>(
  schema: T,
  typeName: K,
): TypeChatJsonValidator<z.TypeOf<T[K]>> {
  ...
  function validate(jsonObject: object) {
    ...
    if (!result.success) {
      return error(
        result.error.issues
          .map(({ path, message }) => `${path.map((key) => `[${JSON.stringify(key)}]`).join("")}: ${message}`)
          .join('"'),
      );
    }
